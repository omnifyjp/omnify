...
export function generateEnumsFile(enums: EnumInfo[]): string {
  ...
  // Use full key pattern: ObjectName_propertyName
  const enumsByKey = new Map<string, EnumInfo>();

  for (const enumInfo of enums) {
    const key = `${enumInfo.objectName}_${enumInfo.propertyName}`;
    enumsByKey.set(key, enumInfo);
  }
  ...
  for (const [key, enumInfo] of enumsByKey.entries()) {
    const enumTypeName = toPascalCase(key);
